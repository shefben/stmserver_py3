...
class MyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def send_compressed_response(self, content, content_type):
        ...
        self.send_header('Content-Length', str(len(content)))#compressed_content)))
        self.end_headers()

        self.wfile.write(content) #compressed_content)
    ...
    def send_gzip_file(self, filepath):
        ...
        if os.path.exists(filepath):
            ...
            self.send_compressed_response(content, content_type)  # Pass content and content_type here
        else:
            self.send_error_response(404, "1File Not Found")
    ...
    def do_POST(self):
        ...
        if self.path.endswith('/'):
            dir_path = os.path.abspath(os.path.join(WEB_ROOT, '.' + self.path))
            index_php_path = os.path.abspath(os.path.join(dir_path, '/index.php'))
            index_html_path = os.path.abspath(os.path.join(dir_path, '/index.html'))

            if os.path.exists(os.path.abspath(index_php_path)):
                self.handle_php_cgi(method='POST', post_data=post_data, accept_encoding=self.headers.get('Accept-Encoding'))
                return
            elif os.path.exists(index_html_path):
                self.send_gzip_file(os.path.abspath(index_html_path))
                return
    ...
    def handle_php_cgi(self, method=None, post_data=None, accept_encoding=None):
        ...
        env = {
            ...
            'DOCUMENT_ROOT': os.path.abspath(WEB_ROOT),
            'GATEWAY_INTERFACE': 'CGI/1.1',
            'SERVER_SOFTWARE': 'StmServer Python/2.7',
            'REDIRECT_STATUS': '200',
        ...
        env['SCRIPT_FILENAME'] = os.path.abspath(os.path.join(WEB_ROOT, '.' + path))
