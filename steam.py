...
def encodeIP(xxx_todo_changeme) :
...
def blob_serialize(blobdict):
    blobtext = b""  # Initialize as bytes
    ...
    for (name, data) in blobdict.items():
        ...
        subtext = subtext + name + data  # No need to convert name to bytes

        if "__slack__" in blobdict:
            slack = blobdict["__slack__"]
        else:
            slack = b""  # Initialize as bytes

        totalsize = len(blobtext) + 10
        sizetext = struct.pack("<LL", totalsize, len(slack))

        blobtext = b"\x01\x50" + sizetext + blobtext + slack  # Convert to bytes
...
class ImpSocket:
    ...
    def send(self, data, log=True):
        ...
        if log:
            logging.debug(str(self.address) + ": Sent data - " + binascii.hexlify(data).decode("utf-8"))
    ...
    def sendto(self, data, address, log=True):
        ...
        if log:
            logging.debug(str(address) + ": sendto Sent data - " + binascii.hexlify(data).decode("utf-8"))
    ...
    def send_withlen(self, data, log=True):
        ...
        if log:
            logging.debug(str(self.address) + ": Sent data with length - " + binascii.hexlify(lengthstr).decode("utf-8") + " " + binascii.hexlify(data).decode("utf-8"))
    ...
    def recv(self, length, log=True):
        ...
        if log:
            logging.debug(str(self.address) + ": Received data - " + binascii.hexlify(data).decode("utf-8"))
    ...
    def recvfrom(self, length, log=True):
        ...
        if log:
            logging.debug(str(address) + ": recvfrom Received data - " + binascii.hexlify(data).decode("utf-8"))
    ...
    def recv_all(self, length, log=True):
        ...
        if log:
            logging.debug(str(self.address) + ": Received all data - " + binascii.hexlify(data).decode("utf-8"))
    ...
    def recv_withlen(self, log=True):
        ...
        if len(lengthstr) != 4:
            ...
            logging.debug(str(self.address) + ": Received data with length  - " + binascii.hexlify(lengthstr).decode("utf-8") + " " + binascii.hexlify(data).decode("utf-8"))
            return data
...
def sign_message(key, message) :
    ...
    checksum_b = SHA.new(phrase_b).digest()
    return checksum_b
def custom_rsa_encrypt(rsakey, plaintext_bytes):
    encrypted = pow(int.from_bytes(plaintext_bytes, byteorder='big'), rsakey.e, rsakey.n)
...
def rsa_sign_message(rsakey, message):
    ...
    fulldigest = b"\x00\x01" + (b"\xff" * 90) + b"\x00\x30\x21\x30\x09\x06\x05\x2b\x0e\x03\x02\x1a\x05\x00\x04\x14" + digest

    # Use custom RSA encryption function
    signature_bytes = custom_rsa_encrypt(rsakey, fulldigest)

    signature = signature_bytes.rjust(128, b"\x00")  # Pad with zeros
...
def rsa_sign_message_1024(rsakey, message):
    ...
    fulldigest = b"\x00\x01" + (b"\xff" * 218) + b"\x00\x30\x21\x30\x09\x06\x05\x2b\x0e\x03\x02\x1a\x05\x00\x04\x14" + digest

    # Use custom RSA encryption function
    signature_bytes = custom_rsa_encrypt(rsakey, fulldigest)

    signature = signature_bytes.rjust(256, b"\x00")  # Pad with zeros
...
def sortfunc(x, y):
    if len(x) == 4 and x[2] == b'\x00':
        if len(y) == 4 and y[2] == b'\x00':
...
def blob_dump(data, spacer=""):
    ...
    blobkeys = list(data.keys())
    blobkeys.sort(key=lambda key: sortfunc(key, data[key]))
    first = True
    for key in blobkeys:
        value = data[key]
        ...
        if isinstance(value, str):
            if first:
                text = text + "\n" + spacer2 + formatstring(key) + ": " + formatstring(value)
                first = False
            else:
                text = text + ",\n" + spacer2 + formatstring(key) + ": " + formatstring(value)
        else:
            if first:
                text = text + "\n" + spacer2 + formatstring(key) + ":\n" + blob_dump(value, spacer2)
                first = False
            else:
                text = text + ",\n" + spacer2 + formatstring(key) + ":\n" + blob_dump(value, spacer2)
