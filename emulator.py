    ...
    def run(self):
        ...
        while True :
            ...
            if self.port == 27010 :
                ...
                if globalvars.data.startswith("1") :
                    ...
                    while i < 1000 : #also change in command "b" and in globalvars
                        if not isinstance(globalvars.hl1serverlist[i], int) :
                            ...
                            trueip_int = list(map(int, trueip))
                ...
                ipstr1 = ipstr.split('\'')
                ipactual = ipstr1[1]
                if globalvars.data.startswith(b"e"):
                    message = binascii.b2a_hex(globalvars.data).decode()  # Convert bytes to str
                    ...
                    if message.startswith("650a01537465616d2e657865"):  # Steam.exe
                        ...
                        f.write(str(binascii.a2b_hex(message[6:24]).decode()))  # Convert bytes to str
                        f.write("\n")
                ...
                elif globalvars.data.startswith(b"c"):
                    message = binascii.b2a_hex(globalvars.data).decode()  # Convert bytes to str
                    ...
                    templist = binascii.a2b_hex(message.encode())  # Convert str to bytes
                    templist2 = templist.split(b'\x00')
                    try:
                        vallist[0] = str(int(binascii.b2a_hex(templist2[0][2:4]).decode(), base=16))
                        vallist[1] = str(int(binascii.b2a_hex(templist2[1]).decode(), base=16))
                        vallist[2] = str(templist2[2].decode())
                        vallist[3] = str(templist2[3].decode())
                        vallist[4] = str(templist2[4].decode())
                        vallist[5] = str(int(binascii.b2a_hex(templist2[5]).decode(), base=16))
                        vallist[6] = str(int(binascii.b2a_hex(templist2[7]).decode(), base=16))
                        vallist[7] = str(int(binascii.b2a_hex(templist2[10]).decode(), base=16))
                        vallist[8] = str(templist2[13].decode())
                        vallist[9] = str(int(binascii.b2a_hex(templist2[14]).decode(), base=16))
                        vallist[10] = str(int(binascii.b2a_hex(templist2[15]).decode(), base=16))
                        vallist[11] = str(int(binascii.b2a_hex(templist2[18]).decode(), base=16))
                        vallist[12] = str(templist2[23].decode())
                ...
                elif globalvars.data.startswith(b"q"):
                    print("Received encrypted ICE client stats - INOP")
                elif globalvars.data.startswith(b"a"):
                    print("Received app download stats - INOP")
                elif globalvars.data.startswith(b"i"):
                    print("Received unknown stats - INOP")
                elif globalvars.data.startswith(b"k"):
...
steam_ver = str(hex_to_int(steam_hex[14:16] + steam_hex[10:12] + steam_hex[6:8] + steam_hex[2:4]))
steamui_hex = firstblob_list[3][25:41]
steamui_ver = hex_to_int(steamui_hex[14:16] + steamui_hex[10:12] + steamui_hex[6:8] + steamui_hex[2:4])
